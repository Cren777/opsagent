...
test('useQuestionSuggestions wires cache, cancellation, fallback, and API calls', () => {
  const sourcePath = path.join(frontendRoot, 'src/composables/useQuestionSuggestions.ts')
  assert.ok(fs.existsSync(sourcePath), 'useQuestionSuggestions.ts must exist')
  const source = fs.readFileSync(sourcePath, 'utf8')
  assert.match(source, /new LatestSuggestionRequest\(\)/)
  assert.match(source, /new SuggestionMemoryCache\(/)
  assert.match(source, /postQuestionSuggestions/)
  assert.match(source, /buildFrontendFallback/)
  assert.match(source, /buildSuggestionCacheKey/)
